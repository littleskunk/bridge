...
FramesRouter.prototype.createFrame = function(req, res, next) {
  ...
  Frame.create(req.user, function(err, frame) {
    if (err) {
      return next(new errors.InternalError('Could not create frame: ' + err.message));
    }
...
FramesRouter.prototype._getContractForShard = function(contr, audit, bl, res, done) {
  ...
  log.debug('Publishing storage contract and wait for offers');
  ...
  self.contracts.load(hash, function(err, item) {
    ...
    self.network.getStorageOffer(contr, bl, function(err, farmer, contract) {
      if (err) {
        return done(new errors.InternalError('Could not get storage offers: ' + err.message));
      }
      ...
      self.contracts.save(item, function(err) {
        if (err) {
          return done(new errors.InternalError('Could not save contract: ' + err.message));
        }
...
FramesRouter.prototype._getContractForShardSIP6 = function(contr, audit, bl, res, done) {
  ...
  log.debug('Select farmers for sending allocs');

  this._selectFarmers(bl, (err, farmers) => {
    if (err) {
      return done(new errors.InternalError('Could not select farmers: ' + err.message));
    }
    ...
    log.debug('Sending allocs to selected farmers %s', JSON.stringify(farmers));

    this._publishContract(farmers, contr, audit, (err, farmerContact, farmerContract, token) => {
      if (err) {
        return done(new errors.InternalError('Could not send allocs: ' + err.message));
      }
  ...
  this.contracts.load(hash, (err, item) => {
    ...
    this.network.publishContract(nodes, contract, (err, data) => {
      ...
      this.contracts.save(item, (err) => {
        if (err) {
          return callback(new errors.InternalError('Could not save contract: ' + err.message));
        }
...
FramesRouter.prototype.addShardToFrame = function(req, res, next) {
  ...
  try {
    ...
    return next(new errors.BadRequestError('Could not add shard to frame: ' + err.message));
  }
  ...
  async.series([
    function checkFrame(done) {
      Frame.findOne({
        ...
        if (err) {
          return done(new errors.InternalError('Could not add shard to frame: ' + err.message));
        }
    ...
    function addPointerToFrame(done) {
      ...
      Pointer.create(pointerData, function(err, pointer) {
        if (err) {
          return done(new errors.BadRequestError('Could not save pointer: ' + err.message));
        }
        ...
        Frame.findOne({
          ...
          if (err) {
            return done(new errors.InternalError('Could not create pointer: ' + err.message));
          }
          ...
          frame.addShard(pointer, (err) => {
            if (err) {
              return done(new errors.InternalError('Could not add shard: ' + err.message));
            }
            res.send({
              hash: req.body.hash,
              token: token,
...
FramesRouter.prototype.destroyFrameById = function(req, res, next) {
  ...
  BucketEntry.findOne({
    ...
    if (err) {
      return next(new errors.InternalError('Could not destroy frame: ' + err.message));
    }
...
FramesRouter.prototype.getFrames = function(req, res, next) {
  ...
  Frame.find({ user: req.user._id }).limit(10).exec(function(err, frames) {
    if (err) {
      return next(new errors.InternalError('Could not get frame: ' + err.message));
    }
